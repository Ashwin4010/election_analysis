...
hdfs dfs -rm -r /output/electionanalysis;
hdfs dfs -mkdir /output/electionanalysis;
##############Start MapReduce program to process  .txt file for State election  ##############################
##################################################################################
echo "********************************************Starting MapReduce program***************************************"
####echo "Started MapReduce program" >> $LOGFILE
####echo "Started MapReduce  - AssemblyYearMinMaxDriver" >> $LOGFILE
echo "********************************************Started MapReduce  - AssemblyYearMinMaxDriver ********************" 
hadoop jar /home/notroot/lab/programs/election/electionanalysis.jar com.sachin.election.driver.AssemblyYearMinMaxDriver /input/India_state_election_data.txt /output/electionanalysis/AC_MIN_MAX_VOTES_T
echo "********************************************Finished MapReduce processing - AssemblyYearMinMaxDriver *********" 
##if [ $? -eq 0 ]; then
##   echo "Finished MapReduce processing - AssemblyYearMinMaxDriver" >> $LOGFILE
##else
##    echo "Failed MapReduce processing job -AssemblyYearMinMaxDriver. Please check Logs " >> $LOGFILE
##fi
###echo "Started MapReduce  - AssemblyStateYearPartyWiseVotesDriver" >> $LOGFILE
echo "********************************************Started MapReduce  - AssemblyStateYearPartyWiseVotesDriver *******" 
hadoop jar /home/notroot/lab/programs/election/electionanalysis.jar com.sachin.election.driver.AssemblyStateYearPartyWiseVotesDriver /input/India_state_election_data.txt /output/electionanalysis/AC_VOTES_T
echo "********************************************Finished MapReduce processing - AssemblyStateYearPartyWiseVotesDriver *******************************"
##if [ $? -eq 0 ]; then
##    echo "Finished MapReduce processing - AssemblyStateYearPartyWiseVotesDriver" >> $LOGFILE
##else
##    echo "Failed MapReduce processing job -AssemblyStateYearPartyWiseVotesDriver. Please check Logs " >> $LOGFILE
##fi

##echo "Started MapReduce  - BottomNConstituencyByStateYearPartyDriver" >> $LOGFILE
echo "********************************************Started MapReduce  - BottomNConstituencyByStateYearPartyDriver ****************************************"
hadoop jar /home/notroot/lab/programs/election/electionanalysis.jar com.sachin.election.driver.BottomNConstituencyByStateYearPartyDriver /input/India_state_election_data.txt /output/electionanalysis/BOTTOM_N_AC_BY_PARTY_T 5
echo "*********************************************Finished MapReduce processing - BottomNConstituencyByStateYearPartyDriver***************************" 
##if [ $? -eq 0 ]; then
##    echo "Finished MapReduce processing - BottomNConstituencyByStateYearPartyDriver" >> $LOGFILE
##else
##    echo "Failed MapReduce processing job -BottomNConstituencyByStateYearPartyDriver. Please check Logs " >> $LOGFILE
##fi
echo "*********************************************Started MapReduce  - TopNConstituencyByStateYearPartyDriver*****************************************"
##echo "Started MapReduce  - TopNConstituencyByStateYearPartyDriver" >> $LOGFILE
hadoop jar /home/notroot/lab/programs/election/electionanalysis.jar com.sachin.election.driver.TopNConstituencyByStateYearPartyDriver /input/India_state_election_data.txt /output/electionanalysis/TOP_N_AC_BY_PARTY_T 5
echo "*********************************************Finished MapReduce processing - TopNConstituencyByStateYearPartyDriver *****************************"
##if [ $? -eq 0 ]; then
##    echo "Finished MapReduce processing - TopNConstituencyByStateYearPartyDriver" >> $LOGFILE
##else
##    echo "Failed MapReduce processing job -TopNConstituencyByStateYearPartyDriver. Please check Logs " >> $LOGFILE
##fi
##echo "Started MapReduce  - VotingPercentByStateYearPartyDriver" >> $LOGFILE
echo "**********************************************Started MapReduce  - VotingPercentByStateYearPartyDriver********************************************"
hadoop jar /home/notroot/lab/programs/election/electionanalysis.jar com.sachin.election.driver.VotingPercentByStateYearPartyDriver /input/India_state_election_data.txt /output/electionanalysis/AC_VOTE_PERCENT_T
echo "**********************************************Finished MapReduce processing - VotingPercentByStateYearPartyDriver*********************************"
##if [ $? -eq 0 ]; then
##    echo "Finished MapReduce processing - VotingPercentByStateYearPartyDriver" >> $LOGFILE
##else
##    echo "Failed MapReduce processing job -VotingPercentByStateYearPartyDriver. Please check Logs " >> $LOGFILE
##fi
#################################################################################
############### Start PIG Processing for Election data  #################################
#################################################################################
echo "************************************Finished MapReduce Processing ***********************************************************************************************"
echo "*********************************************************************************************************************************************************"
##echo "Started PIG Processing " >> $LOGFILE
echo "****************************Started PIG Processing********************************************************************************************* "
pig -f /home/notroot/lab/script/ElectionAnalysis.pig 
echo "****************************Finished PIG processing successfully ******************************************************************************"
##if [ $? -eq 0 ]; then
##    echo "Finished PIG processing successfully " >> $LOGFILE
##else
...
sqoop export --connect jdbc:mysql://localhost/ElectionAnalysis -username root -password admin --table AC_MIN_MAX_VOTES_T --export-dir /output/electionanalysis/pig/AC_MIN_MAX_VOTES_T/part-r-00000 --fields-terminated-by '~';
sqoop export --connect jdbc:mysql://localhost/ElectionAnalysis -username root -password admin --table AC_VOTES_T --export-dir /output/electionanalysis/pig/AC_VOTES_T/part-r-00000 --fields-terminated-by '~';
sqoop export --connect jdbc:mysql://localhost/ElectionAnalysis -username root -password admin --table BOTTOM_N_AC_BY_PARTY_T --export-dir /output/electionanalysis/pig/BOTTOM_N_AC_BY_PARTY_T/part-r-00000 --fields-terminated-by '~';
sqoop export --connect jdbc:mysql://localhost/ElectionAnalysis -username root -password admin --table TOP_N_AC_BY_PARTY_T --export-dir /output/electionanalysis/pig/TOP_N_AC_BY_PARTY_T/part-r-00000 --fields-terminated-by '~';
sqoop export --connect jdbc:mysql://localhost/ElectionAnalysis -username root -password admin --table AC_VOTE_PERCENT_T --export-dir /output/electionanalysis/pig/AC_VOTE_PERCENT_T/part-r-00000 --fields-terminated-by '~';
##if[$? -eq 0]
...
##echo "Started creating of hive tables  " >> $LOGFILE
